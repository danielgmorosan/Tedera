...
export function usePropertyPurchase() {
  ...
  const purchaseShares = async (
    ...
    const availableSharesTokens = parseFloat(
      ethers.utils.formatUnits(availableSharesWei, 18)
    );
    ...
    const totalCost = sharesInWei
      .mul(pricePerWholeTokenInWei)
    ...
    console.log(
      "💰 Price per whole token (from contract):",
      pricePerWholeTokenInWei.toString(),
      "wei"
    );
    console.log(
      "💰 Price per whole token:",
      ethers.utils.formatEther(pricePerWholeTokenInWei),
      "HBAR"
    );
